...
pygame.display.set_caption("\u96d9\u4eba\u8cfd\u8eca")
...
class Player(pygame.sprite.Sprite):
    ...
    def handle_input(self, keys, enemies):
        ...
        if keys[self.keys["up"]] and not self.last_key_states[self.keys["up"]]:
            self.vert_speed_level = min(self.max_speed, self.vert_speed_level + 1)
        if keys[self.keys["down"]] and not self.last_key_states[self.keys["down"]]:
            self.vert_speed_level = max(0, self.vert_speed_level - 1)

        # 移動
        self.rect.x += self.horiz_speed_value * self.speed_unit
        self.rect.y -= self.vert_speed_level * self.speed_unit
...
while running:
    ...
    while not restart_game:
        ...
        for i, player in enumerate([p1, p2]):
            ...
            score_text = small_font.render(f"Distance: {player.score} m", True, WHITE)
            speed_text = small_font.render(f"V-Speed: {player.vert_speed_level}  H-Speed: {player.horiz_speed_value}", True, WHITE)
